...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8959798352:AAGwqUGxrdbAdmeNo9M53oMyYf3d65naagA")
...
# Runtime health / recovery settings
STATE_TIMEOUT_SECONDS = 15 * 60
...
# Premium custom emoji icons for Telegram buttons
BUTTON_EMOJI_IDS = {
    "join": "5210952531676504517",
    "joined": "5416081784641168838",
    "create_post": "6129639980387015660",
    "check_post": "6238007481348397469",
    "statistics": "6129801569941592173",
    "scheduled_posts": "5440621591387980068",
    "channels": "6129532314146838421",
    "profile_reply": "6129650743575060215",
    "leaderboard": "6244492465353529537",
    "profile_inline": "6334797550071060523",
}
...
def reply_btn(
    ...
    """Create a reply-keyboard button with optional color and Premium icon."""
    button: Dict[str, Any] = {"text": text}
    if style in ("success", "primary", "danger"):
        button["style"] = style
    if icon_custom_emoji_id:
        button["icon_custom_emoji_id"] = str(icon_custom_emoji_id)
    return button
...
def main_reply_keyboard() -> Dict[str, Any]:
    return {
        "keyboard": [
            [
                reply_btn("Create Post", "success", BUTTON_EMOJI_IDS["create_post"]),
                reply_btn("Check Post", "primary", BUTTON_EMOJI_IDS["check_post"]),
            ],
            [
                reply_btn("Scheduled Posts", "primary", BUTTON_EMOJI_IDS["scheduled_posts"]),
                reply_btn("Statistics", "primary", BUTTON_EMOJI_IDS["statistics"]),
            ],
            [
                reply_btn("Leaderboard", "primary", BUTTON_EMOJI_IDS["leaderboard"]),
                reply_btn("Channels", "primary", BUTTON_EMOJI_IDS["channels"]),
            ],
            [
                reply_btn("Profile", "primary", BUTTON_EMOJI_IDS["profile_reply"]),
                reply_btn("Add Emoji", "success"),
            ],
            [reply_btn("Owner", "primary")],
        ],
...
def infer_button_style(text: str, callback_data: Optional[str] = None) -> str:
    """Choose a Telegram button style without changing existing callback logic."""
    value = f"{text} {callback_data or ''}".lower()

    danger_words = (
        "delete", "remove", "cancel", "close", "off", "back", "❌", "🗑", "➖"
    )
    success_words = (
        "add", "create", "send", "save", "verify", "check joined", "join", "confirm",
...
def btn(
    ...
    chosen_style = style or infer_button_style(text, callback_data)
    if chosen_style in ("success", "primary", "danger"):
        b["style"] = chosen_style
    if icon_custom_emoji_id:
...
def join_keyboard(channels: List[Dict[str, Any]]):
    ...
    for ch in channels:
        ...
        buttons.append(btn("JOIN", url=url, style="success", icon_custom_emoji_id=BUTTON_EMOJI_IDS["join"]))
    rows = pair_rows(buttons)
    rows.append([btn("Check Joined", "check_joined", style="success", icon_custom_emoji_id=BUTTON_EMOJI_IDS["joined"])])
    return ik(rows)
...
def main_keyboard():
    return ik([
        [
            btn("Create Post", "create_post", style="success", icon_custom_emoji_id=BUTTON_EMOJI_IDS["create_post"]),
            btn("Check Post", "check_post", style="primary", icon_custom_emoji_id=BUTTON_EMOJI_IDS["check_post"]),
        ],
        [
            btn("Profile", "profile", style="primary", icon_custom_emoji_id=BUTTON_EMOJI_IDS["profile_inline"]),
            btn("Add Emoji", "add_emoji", style="success"),
        ],
    ])
...
def admin_keyboard():
    return ik([
        ...
        [btn("Statistics", "admin_stats", icon_custom_emoji_id=BUTTON_EMOJI_IDS["statistics"]), btn("�� Welcome Msg", "admin_set_welcome")],
...
def channel_select_keyboard(user_id: int):
    ...
    for ch in channels:
        ...
        buttons.append(btn(str(title), f"select_post_channel_{ch['channel_id']}", icon_custom_emoji_id=BUTTON_EMOJI_IDS["channels"]))
    rows = pair_rows(buttons)
...
# ===================== HEALTH / RECOVERY =====================
def touch_state(user_id: int, state: Dict[str, Any]):
...
def cleanup_stale_states():
    ...
    for uid, state in list(STATES.items()):
        updated = int(state.get("updated_at") or now)
        if now - updated > STATE_TIMEOUT_SECONDS:
            stale.append(uid)
    for uid in stale:
        STATES.pop(uid, None)
    if stale:
        log(f"Cleared {len(stale)} stale user state(s)")


def recover_to_menu(chat_id: int, user_id: int, message: str = "Session reset ho gayi. Main menu dobara open kar diya hai."):
...
def watchdog_loop():
    ...
    while not STOP_WATCHDOG:
        try:
            gap = time.monotonic() - LAST_LOOP_HEARTBEAT
            if gap > 20:
                log(f"WATCHDOG: polling loop heartbeat delayed by {gap:.1f}s")
            cleanup_stale_states()
        except Exception as e:
            log_error("watchdog_loop", e)
        time.sleep(HEALTH_CHECK_INTERVAL)


def restart_process(reason: str):
    log(f"Restarting bot process safely: {reason}")
    try:
        sys.stdout.flush()
        sys.stderr.flush()
    except Exception:
...
def handle_callback(cb: Dict[str, Any], bot_id: int):
    ...
    if data == "add_channel" or data == "channels_add":
        touch_state(uid, {"name": "waiting_for_channel"})
    ...
    if data == "create_post":
        ...
        if not get_user_emojis(uid):
            ...
            return
        touch_state(uid, {"name": "waiting_for_post"})
    ...
    if data == "add_emoji":
        touch_state(uid, {"name": "waiting_for_emoji"})
    ...
    if data == "send_post":
        ...
        if st.get("name") != "preview":
            answer_callback(cb["id"], "Session expire ho gayi. Main menu open kar raha hoon.", True)
            recover_to_menu(chat_id, uid)
            return
    ...
    if data == "admin_add_force":
        touch_state(uid, {"name": "waiting_for_force_emoji"})
    ...
    if data == "admin_broadcast":
        touch_state(uid, {"name": "waiting_for_broadcast"})
    ...
    if data == "admin_set_update_channel":
        touch_state(uid, {"name": "waiting_update_channel"})
    ...
    if data == "admin_add_admin":
        touch_state(uid, {"name": "waiting_for_new_admin"})
    ...
    if data == "admin_add_emoji":
        touch_state(uid, {"name": "waiting_for_admin_emoji"})
...
def handle_update(update: Dict[str, Any], bot_id: int):
    try:
        ...
        if text.startswith("/start"):
            ...
            return
        if text.startswith("/menu") or text.startswith("/restart"):
            recover_to_menu(message["chat"]["id"], uid, "Main menu refresh ho gaya. Ab koi option select karo.")
            return
        ...
        if STATES.get(uid):
            STATES[uid]["updated_at"] = int(time.time())
        ...
        try:
            ...
            if chat_id:
                recover_to_menu(chat_id, safe_get(update, "message", "from", "id") or safe_get(update, "callback_query", "from", "id") or chat_id, "Temporary error aaya tha. Data safe hai; main menu dobara open kar diya hai.")
        except Exception:
            pass
...
def main():
    ...
    global LAST_LOOP_HEARTBEAT, STOP_WATCHDOG
    watchdog = threading.Thread(target=watchdog_loop, name="bot-watchdog", daemon=True)
    watchdog.start()
    ...
    while True:
        try:
            LAST_LOOP_HEARTBEAT = time.monotonic()
            # Long polling keeps CPU low while still reacting quickly. Telegram
            # returns immediately when a message arrives.
            res = api_call("getUpdates", {"offset": offset, "timeout": 2, "allowed_updates": ["message", "callback_query"]}, timeout=8)
            LAST_LOOP_HEARTBEAT = time.monotonic()
            if not res.get("ok"):
                consecutive_failures += 1
                log(f"getUpdates failed ({consecutive_failures}): {res.get('description')}")
                time.sleep(min(1 + consecutive_failures, 8))
                if consecutive_failures >= MAX_CONSECUTIVE_POLL_FAILURES:
                    restart_process("repeated Telegram polling failures")
                continue
            ...
            time.sleep(min(1 + consecutive_failures, 8))
            if consecutive_failures >= MAX_CONSECUTIVE_POLL_FAILURES:
                restart_process("repeated main loop errors")
